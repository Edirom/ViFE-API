...
import module namespace api="http://xquery.edirom.de/modules/api" at "../../modules/api.xqm";

(: ********** Change this line to point at your swagger config file ********** :)
declare variable $local:swagger-config := json-doc('/db/apps/EdiromOnline/api/v1/swagger.json');

(: ********** Change this if you are using a different prefix for your module in line 19 ********** :)
...
declare variable $local:INVALID_PARAMETER := QName("http://api.edirom.de", "ParameterError");
    ...
    return
        element {$root} {
            ...
            return 
                ...
                case array(*) return <array>{ for $j in $i?* return local:serialize-xml($j, 'root')/node() }</array>
...
return (
    ...
    else local:serialize-json($response())
)